...
| SM-019 | Delegated parent handoff   | `npm run test:e2e:real-smoke` enables multitasking after a real parent exists, then sends the explicit `deck_delegate` instruction | Parent-only task status reaches `completed`; no child session or controls are rendered                  | M5    |

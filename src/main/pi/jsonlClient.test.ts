...
import { JsonlRpcClient, JsonlRpcError, spawnJsonlRpcClient } from "./jsonlClient.js";
...
function spawnFake(args: string[] = []): JsonlRpcClient {
  return spawnJsonlRpcClient(process.execPath, [fakePath(), ...args], { cwd: process.cwd(), env: process.env }, { requestTimeoutMs: 500 });
}

function waitForEvent(client: JsonlRpcClient, predicate: (event: RpcEventRecord) => boolean, timeoutMs = 1_000): Promise<RpcEventRecord> {
...
test("JSONL RPC client rejects pending requests when subprocess exits", async () => {
  ...
  await assert.rejects(client.request("get_state"), /exited|not writable|subprocess/i);
  assert.equal(client.pendingCount, 0);
});
...
test("JSONL RPC client times out command responses and clears pending request", async () => {
  ...
  try {
    await assert.rejects(client.request("get_state", undefined, 20), /timed out/i);
    assert.equal(client.pendingCount, 0);
  } finally {
    client.close();
  }
...
test("JSONL RPC client treats malformed output as fatal and emits parse error event", async () => {
  ...
  const parseError = waitForEvent(client, (event) => event.type === "rpc_parse_error");

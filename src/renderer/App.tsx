...
type CanonicalWorkflowMutationAction = "Retry" | "Stop" | "Answer";
type WorkflowMutationStateUpdater<T> = (update: (current: T) => T) => void;

interface CanonicalWorkflowMutationOptions {
  requestRef: { current: number };
  action: CanonicalWorkflowMutationAction;
  mutate(): Promise<WorkflowRunEnvelope>;
  refresh?(): Promise<WorkflowRunEnvelope>;
  setRuns: WorkflowMutationStateUpdater<WorkflowRunEnvelope[]>;
  setError: WorkflowMutationStateUpdater<string | undefined>;
}

/**
 * Serialize renderer ownership of canonical mutation completions. Main still
 * serializes the persisted run, but IPC calls can resolve in any order here;
 * only the latest renderer request may publish its run or error.
 */
async function executeCanonicalWorkflowMutation(
  options: CanonicalWorkflowMutationOptions,
): Promise<void> {
  const request = ++options.requestRef.current;
  try {
    const run = await options.mutate();
    options.setRuns((current) =>
      options.requestRef.current === request
        ? upsertCanonicalWorkflowRun(current, run)
        : current,
    );
    options.setError((current) =>
      options.requestRef.current === request ? undefined : current,
    );
  } catch (error) {
...
export function App(): ReactElement {
  ...
  // A failed stale canonical mutation must not surface after a newer
  // workflow mutation succeeds while its refresh request is still in flight.
  ...
  async function reconcileStaleWorkspaceSelection(
    ...
    const targetWorkspaceId = staleWorkspaceSelectionCompensation({
      currentGeneration: navigationGeneration.current,
      requestedGeneration: generation,
      requestedWorkspaceId,
      latestWorkspaceSelectionTarget: latestWorkspaceSelectionTarget.current,
    });
  ...
  async function handleSelectSession(sessionId: string): Promise<void> {
    ...
    // the user came from.
    const session = sessionsRef.current.find((item) => item.id === sessionId);
    const navigation = planSessionWorkspaceNavigation({
      generation,
      primaryView,
      activeWorkspaceId: currentWorkspaceRef.current.id,
      ownerWorkspaceId: session?.workspaceId ?? currentWorkspaceRef.current.id,
    });
    const { origin } = navigation;
    if (session?.isResuming === true) {
      return;
    }
    if (session !== undefined && navigation.requiresWorkspaceActivation) {
  ...
  async function openActivityItem(item: ActivityItem): Promise<void> {
    ...
    const navigation = planSessionWorkspaceNavigation({
      ...
      primaryView,
      activeWorkspaceId: currentWorkspaceRef.current.id,
      ownerWorkspaceId: item.workspaceId,
    });
    const { origin } = navigation;
    ...
    if (navigation.requiresWorkspaceActivation) {
      try {
        if (hasWorkspaceApi(window.piDeck)) {
  ...
  async function resumeSession(
    ...
    try {
      ...
      const resumed = resumedSessionForCurrentSavedRow(
        {
          ...sessionFromSnapshot(snapshot),
          workspaceId: session.workspaceId,
        },
        currentSessionForSavedResume(sessionsRef.current, session),
      );
      ...
      if (isNavigationCurrent(generation)) {
        setUiMessage(
          resumed.failureKind === "auth-required"
            ? "Reopened saved Pi session. Authentication verification is pending; send an explicit new prompt after repairing Pi login."
            : transferred
            ...
            >
              {workflowLoading && workflowDefinitions.length === 0 ? (
                ...
                <WorkflowOccurrenceRunView
                  ...
                  onStop={() =>
                    executeCanonicalWorkflowMutation({
                      requestRef: workflowMutationRequest,
                      action: "Stop",
                      mutate: () =>
                  ...
                  onAnswer={(occurrenceId, value) =>
                    executeCanonicalWorkflowMutation({
                      requestRef: workflowMutationRequest,
                      action: "Answer",
                      mutate: () =>
                        window.piDeck.workflows.canonicalAnswerHuman({
                          runId: selectedWorkflowOccurrenceRun.id,
                          occurrenceId,
                          value,
                          expectedRevision:
...
function resumedSessionForCurrentSavedRow(
  ...
  const authStillPending =
    currentSession.failureKind === "auth-required" &&
    resumed.failureKind !== "auth-required" &&
    resumed.authVerified !== true;
  const originalDiagnostic = currentSession.lastError;
  const timeline =
    authStillPending &&
    originalDiagnostic !== undefined &&
    !resumed.timeline.some(
      (item) =>
        item.kind === "diagnostic" && item.content === originalDiagnostic,
    )
      ? [
          ...resumed.timeline,
          {
            id: createId("auth-diagnostic"),
            kind: "diagnostic" as const,
            tone: "error" as const,
            content: originalDiagnostic,
            createdAt: formatTime(),
          },
        ]
      : resumed.timeline;
  return {
    ...
    ...(authStillPending
      ? {
          ...
          failureKind: "auth-required" as const,
          ...(originalDiagnostic !== undefined
...
function sessionFromSnapshot(snapshot: ChatSnapshot): SessionViewModel {
  ...
  if (authRecovery.failure !== undefined) {
    timeline.push({
      id: `auth-diagnostic-${snapshot.runtimeId}`,
      kind: "diagnostic",
      tone: "error",
      content: authRecovery.failure.diagnostic,
      createdAt: formatTime(),
    });
  }
  ...
  const session: SessionViewModel = {
    ...
    ...(authRecovery.failure !== undefined
      ? {
          providerErrorObserved: true,
          failureKind: "auth-required" as const,
          lastError: authRecovery.failure.diagnostic,
        }
      : {}),
    ...(authRecovery.authenticated ? { authVerified: true } : {}),
...
function authRecoveryFromSnapshotMessages(messages: readonly ChatMessage[]): {
  failure?: { diagnostic: string };
  authenticated: boolean;
} {
  // A worker spawn/get_state proves only transport availability. Walk backward
  // through terminal assistant turns: an explicit successful completion is the
  // only durable proof that repaired credentials actually worked.
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const message = messages[index];
    if (message?.role !== "assistant") continue;
    const record = message as Record<string, unknown>;
    if (isErrorAssistantMessage(record)) {
      if (
        classifyOpenAiCodexAuthFailure({
          type: "agent_end",
          messages: [record],
        ...
        return {
          failure: {
            diagnostic:
              getAssistantMessageErrorMessage(record) ?? "Pi agent failed.",
          },
          authenticated: false,
        };
      }
      // A later non-auth failure did not authenticate the model. Continue so

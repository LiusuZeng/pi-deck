...
# Launch Pi Deck in development mode against a real local Pi RPC worker.
# Renderer/style edits are served by Vite and should hot-reload. In dev only,
# Cmd+R/Ctrl+R reloads the renderer and Cmd+Shift+R/Ctrl+Shift+R hard reloads.
# Main/preload changes still require restarting this script.
#
# Usage:
...
echo "Launching Pi Deck dev mode with real Pi backend"
echo "  Repo:    $REPO_ROOT"
echo "  Project: $PROJECT_CWD"
echo "  Pi:      $PI_BINARY"
echo "  Reload:  renderer hot reload via Vite; Cmd+R/Ctrl+R reloads renderer"
echo
